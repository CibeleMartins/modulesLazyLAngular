...
export class CryptoCoinsComponent {
  ...
  ngOnInit() {
    ...
    this.cryptoCoinService.getCryptosAPI().subscribe({
      ...
      complete: () => {console.log('Requisição API BIT Preço concluída com sucesso!')
      return this.graphicCryptos =  new Chart('canvas-cryptos', {
        ...
        data: {
          ...
          datasets: [
            ...
            {
              label: 'Relação % entre o nosso preço mais alto e o nosso preço mais baixo nas últimas 24 horas',
              data: this.arrayTeste.map((i)=> i.market.sell),
              borderColor: '#f0f',
              fill: false,
              tension: 0.8,
            ...
            tooltip: {
              ...
              callbacks: {
                label: (tooltipItem)=> {
                  ...
                  let value = tooltipItem.chart.data.datasets.map((i, index) => index === 3 ? tooltipItem.chart.data.datasets[3].label  + ': ' + i.data[tooltipItem.dataIndex] : 0 );
                  return "" + value;
      
                },
                beforeTitle: (tooltipItem)=> {
                //   console.log(tooltipItem)
                //  let value = tooltipItem[0].chart.data.datasets.map((i, index) => index === 1 ? tooltipItem[0].chart.data.datasets[1].label  + ': ' + i.data[tooltipItem[0].dataIndex] : 0 );
                //  return "" + value;
                },
                title: (tooltipItem)=> {
                
                  // let value = tooltipItem[0].chart.data.datasets.map((i, index) => index === 2 ? tooltipItem[0].chart.data.datasets[2].label  + ': ' + i.data[tooltipItem[0].dataIndex] : 0 );
                  // return "" + value;
                },
                ...
                beforeBody: (tooltipItem)=> {
                  // console.log(tooltipItem)
                  let value = tooltipItem[0].chart.data.datasets.map((i, index) => index === 0 ? tooltipItem[0].chart.data.datasets[0].label  + ': ' + i.data[tooltipItem[0].dataIndex] : 0 );
                  return "" + value;
                },
                ...
                beforeLabel:(tooltipItem)=> {
                  ...
                  let value = tooltipItem.chart.data.datasets.map((i, index) => index === 5 ? tooltipItem.chart.data.datasets[5].label  + ': ' + i.data[tooltipItem.dataIndex] : 0 );
